...
def update_dimensionSelection(input_data):
    ...
    if input_data is not None:
        try:
            ...
            ddDim2 = ddDim1 
            ddDim3 = ddDim1
            return ddDim1, ddDim2, ddDim3
        except:
            return [{'label':'Dimension 0', 'value':0}],[{'label':'Dimension 0', 'value':0}],[{'label':'Dimension 0', 'value':0}]
    else:
        return [{'label':'Dimension 0', 'value':0}],[{'label':'Dimension 0', 'value':0}],[{'label':'Dimension 0', 'value':0}]
...
@app.callback([Output('manifold-data-storage', 'children'),
               Output('dropdownDimension1','value'),
               Output('dropdownDimension2','value')],
...
def update_output_div(n_clicks, input_data, manifold, ncomponents):
    try:
        if(input_data is not None):
            ...
            return principalDf.to_json(date_format='iso',orient = 'split'), 'Principal component 0', 'Principal component 1'
    except:
        return [[],'','']

@app.callback(Output('graphPCA','children'),
             [Input('filtered-data-storage','children')])
def pca_varianze (input_data):
   if input_data is not None:
     dff = pd.read_json(input_data[0],orient='split')
     pca = PCA()
